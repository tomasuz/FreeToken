...
def register_openai_routes(
    ...
    async def v1_models():
        ...
        has_mtp = getattr(state.config, "mtp", False) or getattr(getattr(state.config, "model_config", None), "has_mtp", False)
        mtp_model = getattr(state.config, "mtp_model_path", None) if has_mtp else None
        mtp_draft = (getattr(state.config, "mtp_draft", None) or getattr(getattr(state.config, "model_config", None), "mtp_draft", None)) if has_mtp else None
