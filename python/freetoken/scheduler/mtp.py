...
class MTPDecodeMixin:
    ...
    def _mtp_head_draft(self, req: Req, prev_hidden: torch.Tensor) -> int | None:
        ...
        try:
            ...
            # The head's MoE is a RESIDENT (fused) MoELayer: Qwen3_5MTPBlock builds it from
            # a config copy with moe_strategy="fused", so its packed experts stay on the GPU
            # and MoELayer.forward runs them itself. The offload backend is never on this
            # path, so there is nothing to swap around the call.
            with self.engine.ctx.forward_batch(b):
