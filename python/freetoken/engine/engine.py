...
class Engine:
    ...
    def _init_offload_moe_cache(self, config: EngineConfig) -> OffloadMoeCache:
        ...
        split_residency = config.moe_strategy in ("offload", "hybrid") and (
            bool(inplace_layers)
            or (
                bool(cpu_layer_ids)
        ...
        # routing concentration is what decides whether ANY cache policy can help;
        # it rides the same diagnostic flag rather than a second one nobody sets
...
def _resolve_worker_layers(
    ...
    """
    if not config.moe_worker_layers or not is_offload_moe_strategy(config.moe_strategy):
        return {}
