...
static __global__ void moe_vec_q(
  ...
  // expert_stride: bytes between experts when the caller hands a strided bank (a slot cache
  // sized for the widest layer); < 0 -> experts packed back to back, as they always were
  const block_q_t* x = expert_stride >= 0
      ? (const block_q_t*)((const char*)vx + (int64_t)expert * expert_stride)
...
#pragma unroll
  for (int mask = WARP_SIZE / 2; mask > 0; mask >>= 1) {
    tmp += SGLANG_SHFL_XOR_SYNC(SGLANG_FULL_MASK, tmp, mask);
  }
...
static void moe_vec_q4_0_q8_1_cuda(
  ...
  const dim3 block_dims(gguf_device_warp_size(), GGML_CUDA_MMV_Y, 1);
  moe_vec_q<scalar_t, QK4_0, QI4_0, block_q4_0, VDR_Q4_0_Q8_1_MMVQ, vec_dot_q4_0_q8_1>
      <<<block_nums, block_dims, 0, stream>>>(vx, vy, dst, topk_ids, top_k, ncols, nrows, token_stride, expert_stride);
}
