...
`ggml/src/ggml-cpu/quants.{c,h}`, `ggml-cpu/arch/x86/quants.c`, `ggml-cpu/arch/arm/quants.c`,
`ggml-cpu-impl.h`,
...
executor uses (and the portable C versions and reference quantizers, for tests);
`freetoken/kernel/ggml_cpu.py` builds the file for the host's CPU family (x86-64 or aarch64)
with the host compiler (`-march=native` / `-mcpu=native`).
